...
int SetupInterruptSystem(XUartLite *UartLitePtr_1, u8 UartLite_1_Id, XUartLite *UartLitePtr_2, u8 UartLite_2_Id, XTmrCtr* TmrCtrInstancePtr, u8 Timer_Id)
{
	 ...
	 */
	Status = XIntc_Connect(&InterruptController, Timer_Id, (XInterruptHandler)XTmrCtr_InterruptHandler, (void *)TmrCtrInstancePtr);
	if (Status != XST_SUCCESS) {
		return XST_FAILURE;
	}
	 ...
	 */
	XIntc_Enable(&InterruptController, Timer_Id);
	/*
